...
@Component({
  ...
  styleUrls: ['./paymentmaster.component.scss'],
...
export class PaymentmasterComponent {
  ...
  Init() {
    ...
    //   this.http.get('payment/payments',this.entity).subscribe({
    //     next: (res: any) => {
    //       if (res.status_cd == 1) {
  ...
  addNew() {
    ...
    this.router.navigate(['Paymentchild'], { state: params });
  }
  onGridReady(params) {
  ...
  edit(s) {
    ...
    this.router.navigate(['Paymentchild'], { state: param });
  ...
  iConfirmFn(row) {
    ...
    this.http.delete('payment/delete', params).subscribe((res: any) => {
      if (res.status_cd == 1) {
