...
@Component({
  ...
  imports: [DssGridComponent, FormsModule, ngselectComponent, CommonModule, DssInputComponent],
...
export class ExpenseComponent {
  ...
  iConfirmFn(row) {
    ...
    this.http.delete('Expense/delete', params).subscribe((res: any) => {
      if (res.status_cd == 1) {
  ...
  save() {
    this.spinner.show();
    if (!this.entity.expenseId) {

      this.http.post('Expense/Create', this.entity).subscribe({
        next: (res: any) => {
          if (res.status_cd == 1) {

            this.entity.expenseId = res.data.expenseId;
      ...
      this.http.put('Expense/Edit', this.entity, { id: this.entity.expenseId }).subscribe({
        next: (res: any) => {
          if (res.status_cd == 1) {
            this.entity.expenseId = res.data.expenseId;
